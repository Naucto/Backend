...
function makeRefreshTokenMock(overrides: Record<string, jest.Mock> = {}) {
  return {
    create: jest.fn().mockResolvedValue({ id: 1, token: "hashed", userId: 1, expiresAt: new Date() }),
...
function makePrisma(refreshTokenOverrides: Record<string, jest.Mock> = {}) {
  return {
    $transaction: jest.fn((cb: any) =>
      cb({
        refreshToken: {
          create: jest.fn().mockResolvedValue({ id: 2, token: "hashed", userId: 1, expiresAt: new Date() }),
          deleteMany: jest.fn().mockResolvedValue({ count: 0 }),
          delete: jest.fn().mockResolvedValue({ id: 1 })
        }
      })
...
describe("AuthService", () => {
  ...
  const jwtService: jest.Mocked<Pick<JwtService, "sign" | "decode" | "verify">> = {
  ...
  beforeEach(async () => {
    ...
    (jwtService.decode as jest.Mock).mockReturnValue({ sub: 1, email: "test@test.com" });
  ...
  describe("validateUser", () => {
    ...
    it("should throw UnauthorizedException if password is invalid", async () => {
      userService.findByEmail.mockResolvedValue({
        ...
        nickname: null, password: "hashedPass", createdAt: new Date()
      });
    ...
    it("should return user if email and password are valid", async () => {
      const mockUser = {
        id: 1, email: "test@example.com", password: "hashedPass",
        username: "testuser", nickname: null, createdAt: new Date()
      };
  ...
  describe("register", () => {
    it("should throw ConflictException if email already exists", async () => {
      userService.findAll.mockImplementation(async (params?: Prisma.UserFindManyArgs): Promise<User[]> => {
        ...
        if (emailFilter === "exists@example.com") {
          return [{ id: 1, email: emailFilter, username: "user", nickname: null, password: "hashedPass", createdAt: new Date() }];
        }
        return [];
      });

      await expect(authService.register({ email: "exists@example.com", username: "user", password: "pass", roles: [] }))
    ...
    it("should throw ConflictException if username already exists", async () => {
      userService.findAll.mockImplementation(async (params?: Prisma.UserFindManyArgs): Promise<User[]> => {
        ...
        if (where.username) {
          if (typeof where.username === "string") usernameFilter = where.username;
        ...
        if (usernameFilter === "existsUser") {
          return [{ id: 2, email: "user@example.com", username: usernameFilter, nickname: null, password: "hashedPass", createdAt: new Date() }];
        }
        return [];
      });

      await expect(authService.register({ email: "new@example.com", username: "existsUser", password: "pass", roles: [] }))
        .rejects.toThrow(ConflictException);
    });
    ...
    it("should create user and return access token", async () => {
      ...
      userService.create.mockResolvedValue({
        ...
        nickname: null, password: "hashedPassword", createdAt: new Date()
      });

      const result = await authService.register({ email: "new@example.com", username: "newUser", password: "pass", roles: [] });
  ...
  describe("loginWithGoogle", () => {
    it("should create new user and return tokens for new Google user", async () => {
      const googleUser = { email: "google@example.com", name: "Google User" };
      const googleAuthService = { verifyToken: jest.fn().mockResolvedValue(googleUser) };

      userService.findByEmail.mockResolvedValue(undefined);
      userService.findAll.mockResolvedValue([]);
      userService.createOAuthUser.mockResolvedValue({
        id: 5, email: googleUser.email, username: "Google_User",
        nickname: null, password: "", createdAt: new Date()
      });

      const svc = await buildModule(prismaService, googleAuthService);
      const result = await svc.loginWithGoogle("google-oauth-token");

      expect(googleAuthService.verifyToken).toHaveBeenCalledWith("google-oauth-token");
      expect(userService.createOAuthUser).toHaveBeenCalledWith(googleUser.email, "Google_User");
      expect(result).toEqual({ access_token: "token123", refresh_token: "token123" });
    });

    it("should return tokens for existing Google user", async () => {
      const googleUser = { email: "existing@example.com", name: "Existing User" };
      const googleAuthService = { verifyToken: jest.fn().mockResolvedValue(googleUser) };

      userService.findByEmail.mockResolvedValue({
        id: 6, email: googleUser.email, username: "existing_user",
        nickname: null, password: "somepass", createdAt: new Date()
      });
  ...
  describe("refreshToken", () => {
    it("should throw UnauthorizedException if refresh token not found", async () => {
      ...
      await expect(svc.refreshToken("invalid-token")).rejects.toThrow(UnauthorizedException);
    });

    it("should throw UnauthorizedException if refresh token expired", async () => {
      const expiredDate = new Date(Date.now() - 1000 * 60 * 60);
      const tokenRecord = {
        id: 1, token: "hashed-expired", userId: 1, expiresAt: expiredDate,
        user: { id: 1, email: "user@example.com", username: "user", nickname: null, password: "pass", createdAt: new Date() }
      };

      (jwtService.verify as jest.Mock).mockReturnValue({ sub: 1, email: "user@example.com" });
      (bcrypt.compare as jest.Mock).mockResolvedValue(true);
      ...
      await expect(svc.refreshToken("expired-token")).rejects.toThrow(UnauthorizedException);
    ...
    it("should return new tokens for valid refresh token", async () => {
      ...
      const tokenRecord = {
        id: 1, token: "hashed-valid", userId: 1, expiresAt: futureDate,
        user: { id: 1, email: "user@example.com", username: "user", nickname: null, password: "pass", createdAt: new Date() }
      };
      ...
      const prisma = makePrisma({ findMany: jest.fn().mockResolvedValue([tokenRecord]) });
      const svc = await buildModule(prisma);

      const result = await svc.refreshToken("valid-token");
      expect(result).toEqual({ access_token: "new-access-token", refresh_token: "new-access-token" });
    });
  });
  ...
  describe("validateUser edge cases", () => {
    it("should throw UnauthorizedException if user has no password", async () => {
      userService.findByEmail.mockResolvedValue({
        id: 1, email: "google@example.com", username: "googleuser",
        nickname: null, password: null, createdAt: new Date()
      } as any);

      await expect(authService.validateUser("google@example.com", "password")).rejects.toThrow(UnauthorizedException);
    });
  });
});
