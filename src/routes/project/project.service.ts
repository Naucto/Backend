...
export class ProjectService {
  ...
  constructor(
    ...
    this.auto_save_delay =
      (configService.get<number>("S3_AUTO_HISTORY_DELAY") ?? 10) * 60000;
  }
  ...
  async remove(id: number): Promise<void> {
    ...
    try {
      await this.s3Service.deleteFile(`release/${id}`);

      const checkpoint_prefix = `checkpoint/${id}/`;
      const checkpoints = await this.s3Service.listObjects(undefined, checkpoint_prefix);
      
      if (checkpoints.length > 0) {
        const objects = checkpoints.map((o) => o.Key!);
        await this.s3Service.deleteFiles(objects);
      }

      const save_prefix = `save/${id}/`;
      const saves = await this.s3Service.listObjects(undefined, save_prefix);
    ...
    if (files.length >= this.max_history_version) {
      ...
      if (last_save_time < this.auto_save_delay) {
        await this.s3Service.deleteFile(
          filename_prefix + (files[0]?.name ?? ""),
        );
      } else {
        await this.s3Service.deleteFile(
           filename_prefix + (files[files.length - 1]?.name ?? ""),
        );
      }
    }
  ...
  async checkpoint(projectId: number, name: string): Promise<void> {
    ...
    await this.s3Service.uploadFile(
      ...
      {}, 
      undefined, 
      `checkpoint/${projectId}/${name}`
    );
  }

  async removeCheckpoint(projectId: number, checkpoint: string): Promise<void> {
    await this.s3Service.deleteFile(`checkpoint/${projectId}/${checkpoint}`);
  }
  ...
  async unpublish(projectId: number): Promise<void> {
    ...
    await this.s3Service.deleteFile(`release/${projectId}`);
  }

  async listVersions(projectId: number): Promise<ProjectSave[]> {
    return (
      await this.s3Service.listObjects(undefined, `save/${projectId}/`)
    ).map((o) => ({ name: o.Key!.split("/").pop()!, date: o.LastModified! }));
  }

  async listCheckpoints(projectId: number): Promise<ProjectSave[]> {
    return (
      await this.s3Service.listObjects(undefined, `checkpoint/${projectId}/`)
    ).map((o) => ({ name: o.Key!.split("/").pop()!, date: o.LastModified! }));
  }
  ...
  async fetchSavedVersion(
    ...
    return this.s3Service.downloadFile(`save/${projectId}/${version}`);
  }
  ...
  async fetchCheckpoint(
    ...
    return this.s3Service.downloadFile(file);
  }
  ...
  async fetchReleaseContent(projectId: number): Promise<DownloadedFile> {
    return this.s3Service.downloadFile(`release/${projectId}`);
  }
