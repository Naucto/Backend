...
export class WorkSessionController {
  ...
  async join(@Param("id", ParseIntPipe) projectId: number, @Req() req: { user: import("@auth/dto/user.dto").UserDto }): Promise<JoinRoomResult> {
  ...
  @ApiResponse({ status: HttpStatus.BAD_REQUEST, description: "Bad request." })
  @ApiParam({ name: "id", description: "Project ID" })
  async leave(@Param("id", ParseIntPipe) projectId: number, @Req() req: { user: import("@auth/dto/user.dto").UserDto }): Promise<void> {
  ...
  @ApiParam({ name: "id", description: "Project ID" })
  async getInfo(@Param("id", ParseIntPipe) workSessionId: number): Promise<FetchWorkSessionDto> {
